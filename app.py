...
def home():
    ...
    return render_template('home.html', group_data=group_data)
...
def groups(class_name):
    if class_name not in group_data:
        flash('Invalid academic year selected', 'error')
        return redirect(url_for('home'))
    
    return render_template('group.html', 
                         class_name=class_name, 
                         groups=group_data[class_name])

@app.route('/fee_table/<class_name>/<group_name>')
@login_required
def fee_table(class_name, group_name):
    # Validate class and group
    if class_name not in group_data or group_name not in group_data[class_name]:
        flash('Invalid class or group selected', 'error')
        return redirect(url_for('home'))
